...
class StyleBased_Generator(nn.Module):
    ...
    def forward(self, 
        ...
        # 초기 마스크: 모든 foreground (1.0)
        mask = torch.ones_like(first_content[:, :1, :, :])  # shape: (B, 1, H, W)
        ...
        for i, conv in enumerate(self.convs):
            ...
            if i > 0 and step > 0:
                result_upsample = nn.functional.interpolate(result, scale_factor=2, mode='bilinear',
                                                            align_corners=False)
                mask_upsample = nn.functional.interpolate(mask, scale_factor=2, mode='bilinear', align_corners=False)
            ...
            # RGB 이미지 출력
            if i == step:
                rgb_out = self.to_rgbs[i](result)
                if i > 0 and 0 <= alpha < 1:
                    rgb_prev = self.to_rgbs[i - 1](result_upsample)
                    rgb_out = alpha * rgb_out + (1 - alpha) * rgb_prev
                
                # 마스크 refinement
                mask_input = torch.cat([result, mask_upsample], dim=1)
                mask_out = torch.sigmoid(self.to_masks[i](mask_input))
                
                break
            else:
                # 마스크 refinement 중간 단계
                mask_input = torch.cat([result, mask_upsample], dim=1)
                mask = torch.sigmoid(self.to_masks[i](mask_input))
